...
class BisectionOop:
    ...
    def __checkInputValidity(self):
        ...
        if self.f(L) == 0 or self.f(R) == 0:
            return 1

        # Check if a < b
        if L > R or (L == R and self.f(L) != 0):
            return 0

        # Check if f(a) * f(b) < 0
        if self.f(L) * self.f(R) >= 0:
            return 0
    ...
    def __bisectionMethod(self):
        ...
        while abs(right - left) >= epsilon:
            ...
            print(left, right, mid, sep=', ', file=sys.stderr)
...
# print(uu.Solve())
# uu = BisectionOop(0.008, 0.08, 1e-10, "1/sqrt(x)+2*log(0.0000015/(3.7*0.005)+ 2.51/(13.743*sqrt(x)))/log(10)")
